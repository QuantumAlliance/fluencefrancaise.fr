<template>
  <!-- Same shape as My Courses, so the portal reads consistently. -->
  <div class="p-8">
    <div class="flex justify-between items-center mb-8">
      <h1 class="text-2xl md:text-3xl font-bold text-gray-800">Exam Prep</h1>
    ...
    <div v-else-if="!examPreps.length" class="text-center py-12 bg-white rounded-lg shadow">
      <p class="text-gray-500">No exam prep found</p>
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      >
        ...
        <div class="p-6">
          <h2 class="text-xl font-bold text-gray-800 mb-2 line-clamp-1">{{ examPrep.title }}</h2>

          <p class="text-gray-600 text-sm mb-4 line-clamp-2">
            {{ examPrep.description || examPrep.subtitle || 'No description available' }}
          </p>
...
const loadExamPreps = async () => {
  ...
  error.value = ''
  try {
    const response = await axios.get('/api/demo/exam-preps?per_page=100')
    examPreps.value = response.data?.data?.data ?? []
  } catch (err) {
    console.error('Failed to load exam prep:', err)
    error.value = 'Unable to load exam prep right now.'
  } finally {
    loading.value = false
  }
}
